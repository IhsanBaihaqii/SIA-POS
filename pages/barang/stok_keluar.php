...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($jumlah <= 0) {
        ...
        $errors[] = 'Stok tidak mencukupi. Stok saat ini: ' . $barang['stok'];
    }
    if (empty($errors)) {
        // Update stok barang
        ...
        // Catat mutasi
        $stmt = $pdo->prepare("INSERT INTO stok_mutasi (barang_id, jenis, jumlah, keterangan, tanggal, user_id) VALUES (?, 'keluar', ?, ?, ?, ?)");
        $stmt->execute([$id, $jumlah, $keterangan, $tanggal, $_SESSION['user_id']]);
        header('Location: ' . url('pages/barang/riwayat_mutasi.php?barang_id=' . $id . '&msg=keluar'));
        exit;
    }
}
...
<div class="mb-6">
    <h2 class="text-2xl font-bold text-gray-800">Stok Keluar - <?= htmlspecialchars($barang['nama']) ?></h2>
    <p class="text-sm text-gray-500">Stok saat ini: <?= $barang['stok'] ?></p>
</div>
...
<form method="POST" class="bg-white shadow rounded-lg p-6 max-w-lg">
    ...
    <button type="submit" class="bg-orange-600 hover:bg-orange-700 text-white font-semibold py-2 px-4 rounded-lg">
        <i class="fa-solid fa-arrow-up mr-2"></i> Proses Stok Keluar
    </button>
